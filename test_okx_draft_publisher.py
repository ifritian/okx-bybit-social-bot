#!/usr/bin/env python3
"""
Тесты okx_draft_publisher.py: is_configured(), формирование
...
def test_is_configured_false_without_chat_id(monkeypatch):
    ...
    monkeypatch.setattr(config, "OKX_ORBIT_DRAFT_CHAT_ID", "")
    assert okx_draft_publisher.is_configured() is False


def test_is_configured_true_with_token_and_chat_id(monkeypatch):
    monkeypatch.setattr(config, "TELEGRAM_BOT_TOKEN", "test-token")
    monkeypatch.setattr(config, "OKX_ORBIT_DRAFT_CHAT_ID", "123456")
    assert okx_draft_publisher.is_configured() is True
...
def test_send_draft_without_image_sends_text_message(monkeypatch):
    ...
    monkeypatch.setattr(okx_draft_publisher.requests, "post", _fake_post)

    result = okx_draft_publisher.send_draft("Текст черновика поста.", "market_take")
...
def test_send_draft_raises_on_api_error(monkeypatch):
    ...
    try:
        okx_draft_publisher.send_draft("Текст.", "trading_insight")
        raise AssertionError("ожидалось DraftDeliveryError")
    except okx_draft_publisher.DraftDeliveryError:
        pass
