"""
bybit_byx_generator.py - генерация постов под Bybit ByX.

Устроено идентично okx_orbit_generator.py (см. комментарии там для
подробностей по логике) - у Bybit ByX та же ситуация: нет публичного
API для публикации, посты проходят модерацию (в интерфейсе видны
статусы Published/Pending/Not Approved), поэтому bybit_draft_publisher.py
только готовит черновик и присылает его владельцу в Telegram.

Два формата (те же, что у OKX):
1. "market_take" - развёрнутое наблюдение о движении рынка.
2. "trading_insight" - короткое конкретное наблюдение по сетапу/уровню.

Реальные числа считает market_stats.calc_theme_stats - тот же общий
модуль, что использует okx_orbit_generator.py, LLM только пишет текст
вокруг готовых цифр.
"""
import logging
import random
...
публикует авторский разбор рынка на Bybit ByX (соцплатформа для крипто-
...
Не упоминай Binance, OKX или любую конкретную биржу. Не добавляй сам
никакой дисклеймер - он будет добавлен отдельно после твоего текста.
...
который публикует короткие трейдинг-инсайты на Bybit ByX (соцплатформа
...
def pick_format(last_format: Optional[str]) -> str:
    """Выбирает формат, отличный от последнего использованного."""
...
def _build_user_prompt(theme: str, stats: dict, task_line: str) -> tuple[str, set[float], float]:
    ...
    if "single" in stats:
        ...
        user_prompt = (
            ...
            f"{task_line}"
            f"{voice_memory.anti_repeat_block('bybit_byx')}"
            f"{voice_memory.continuity_block('bybit_byx', theme, label)}"
        )
        # 48 и 2 разрешены отдельно - промпт сам просит модель писать
...
def generate_bybit_byx_post(theme: str, format_type: str) -> Optional[tuple]:
    ...
    if len(hook.strip()) < 10:
        logger.warning("Хук Bybit ByX-поста пустой или слишком короткий (%r) - пропускаю окно", hook)
        return None

    text = f"{hook.strip()}\n\n{DISCLAIMER}"
    logger.info("Сгенерирован Bybit ByX-пост (тема %s, формат %s, числа: %s)", theme, format_type, allowed_numbers)
    return text, allowed_numbers, headline_pct, format_type


def validate_bybit_byx_post_text(text: str, allowed_numbers: set[float]) -> tuple[bool, str]:
    """Числа только из посчитанных данных, дисклеймер на месте, без
    ИИ-штампов. Идентична okx_orbit_generator.validate_okx_orbit_post_text."""
    import re
...
def generate_chart_for_post(theme: str) -> Optional[Path]:
    ...
    BINANCE - см. okx_orbit_generator.generate_chart_for_post."""
    ticker = THEMES[theme]["tickers"][0]
    return generate_chart_image(ticker, days=2, watermark_text=None, filename_suffix="_bybit")
